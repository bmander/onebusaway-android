...
class DefaultRegionRepository @Inject constructor(
    ...
    override suspend fun refresh(): RegionStatus = withContext(Dispatchers.IO) {
        ...
        if (results == null) {
            ...
            // couldn't be loaded from any source. Surface it (the region/ and ui/home/ packages
            // otherwise log nothing) and record a non-fatal so the failure rate is trackable; the
            // Failed state drives a retryable affordance at the picker host.
