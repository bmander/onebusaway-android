 ...
 * [MyStopsActivity]'s starred tab (`tab://starred`), then finishes. Kept as its own manifest entry
 * so shortcuts pinned by older versions keep resolving.
 */
class MyStarredStopsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.action == Intent.ACTION_CREATE_SHORTCUT) {
            val shortcut = UIUtils.makeShortcutInfo(
                ...
                Intent(this, MyStopsActivity::class.java).setData(MyTabs.defaultTabUri(MyTabs.STARRED_STOPS)),
