 ...
 * results. Kept a pure sealed type (no Android dependencies) so [TripMonitorDecider.decide] is
...
sealed interface MonitorResult {
    ...
    /** The itinerary still matches within the threshold and the trip hasn't ended — keep polling. */
    data object KeepMonitoring : MonitorResult

    /**
     * Monitoring should stop without notifying: either the plan returned no results (the legacy
     * "failure → disable" path) or the monitored trip has already ended.
     */
    data object Stop : MonitorResult
}

/**
 * Pure decision function for the trip-plan-change monitor. Mirrors the branch structure of the
 * legacy `RealtimeService.checkForItineraryChange`, factored out so it can be exercised without a
 * running service or Android framework.
 */
object TripMonitorDecider {
    ...
    fun decide(
        ...
        for (itinerary in results) {
            ...
            // Found the itinerary the user is on.
            val delay = current.getDelay(other)
            if (abs(delay) > thresholdSeconds) {
                return MonitorResult.Deviation(delay)
            }
            // Still on plan; stop once the trip has ended, otherwise keep polling.
            return if (current.isExpired()) MonitorResult.Stop else MonitorResult.KeepMonitoring
        }
...
/** Pure timing helper for when active monitoring should begin (extracted for JVM testing). */
