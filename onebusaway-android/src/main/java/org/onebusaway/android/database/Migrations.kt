...
val MIGRATION_2_3 = object : Migration(2, 3) {
    override fun migrate(db: SupportSQLiteDatabase) {
        ...
        db.execSQL(
            "CREATE TABLE IF NOT EXISTS `regions` (`_id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, " +
                ...
                "`payment_warning_body` TEXT, `travel_behavior_data_collection` INTEGER, " +
                "`enroll_participants_in_study` INTEGER, " +
                "`sidecar_base_url` TEXT DEFAULT 'https://onebusaway.co', " +
                "`plausible_analytics_server_url` TEXT, `umami_analytics_url` TEXT, " +
                "`umami_analytics_id` TEXT)"
        )
        db.execSQL(
