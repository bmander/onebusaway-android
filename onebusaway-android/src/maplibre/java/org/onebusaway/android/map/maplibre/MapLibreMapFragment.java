...
/**
 * Thin {@link Fragment} wrapper around {@link MapLibreMapHost}. Home hosts the map directly via
 * {@link ObaMapHost} (no FragmentManager); this wrapper keeps the other map screens
 * ({@code TripResultsFragment}, {@code InfrastructureIssueActivity}, {@code TripPlanLocationPicker})
 * working unchanged — it owns a {@link MapLibreMapHost}, forwards the Fragment lifecycle to it (now
 * including {@code onStart/onStop/onLowMemory}, which the SDK's {@code SupportMapFragment} used to
 * handle), drives the location-permission request through an {@link ActivityResultLauncher} (as
 * {@link MapHostDeps}), and delegates the {@link ObaMapFragment} contract to the host.
 *
 ...
 */
public class MapLibreMapFragment extends Fragment implements ObaMapFragment, MapHostDeps {

    /** Kept for {@link StopOverlay} (the zoom threshold for showing stops). */
    public static final float CAMERA_DEFAULT_ZOOM = MapLibreMapHost.CAMERA_DEFAULT_ZOOM;

    public static final float DEFAULT_MAP_PADDING_DP = MapLibreMapHost.DEFAULT_MAP_PADDING_DP;

    private MapLibreMapHost mHost;
    ...
    public static MapLibreMapFragment newInstance() {
        return new MapLibreMapFragment();
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mHost = new MapLibreMapHost(requireActivity(), this, args);
