 ...
 * superseded. This is the coroutine-loop replacement for the legacy `RealtimeService` background
 * poll (`AlarmManager.setInexactRepeating(60s)` → `WakefulBroadcastReceiver` → `IntentService`).
 *
 ...
 * the plan returns nothing, or the monitored trip ends, so the foreground notification is short-lived.
 */
@AndroidEntryPoint
class TripPlanMonitorService : Service() {
    ...
    private suspend fun runMonitorLoop(
        ...
        try {
            while (coroutineContext.isActive) {
                ...
                Log.d(TAG, "Monitor check result: $result")
                when (result) {
    ...
    private fun buildOngoingNotification(target: Class<*>): Notification {
        ...
        return NotificationCompat.Builder(this, Application.CHANNEL_TRIP_PLAN_UPDATES_ID)
    ...
    private fun notifyChange(
        ...
        // pre-O heads-up. (The legacy service set the now-deprecated Notification.defaults / FLAG_SHOW_LIGHTS.)
        val notification = NotificationCompat.Builder(this, Application.CHANNEL_TRIP_PLAN_UPDATES_ID)
            .setSmallIcon(R.drawable.ic_stat_notification)
    ...
    private fun readItineraryDescription(extras: Bundle): ItineraryDescription? {
        ...
        return ItineraryDescription(tripIds, Date(endDateMillis))
    }
