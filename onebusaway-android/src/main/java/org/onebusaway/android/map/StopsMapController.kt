...
class StopsMapController(
    ...
    private fun onStopsLoaded(result: Result<NearbyStops?>) {
        ...
        if (nearby == null) {
            // No endpoint yet (or a null no-op); no stops loaded, so clear the banner and do nothing
            // (#615) — like the other early-outs, keep moreStopsAvailable in step with each load.
            host.setMoreStopsAvailable(false)
            return
        }
