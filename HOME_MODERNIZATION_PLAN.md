...
**End state (as achieved through Phase 2 — the original "≈120 lines / Activity holds no UI state /
`onSaveInstanceState` gone" targets were over-claims; here is the honest result):**
`HomeActivity.kt` is a ~1,100-line host (down from 2,227) whose remaining bulk is *legitimate* Activity work — intent
parsing + `start()`/`makeIntent()` entry points, the imperative map host's creation/lifecycle, the
location-permission `ActivityResultLauncher`, drawer-item building, applying `HomeEvent`s to the map,
and the still-View survey wiring (its own campaign). It holds **no screen-coordination state**: the
fully-tested `HomeViewModel` is the single source of truth (nav selection, focus, sheet-settled,
pending-focus, chrome/overlay/dialog gates, weather/alerts/region), exposed as one `StateFlow<HomeUiState>`
with one-shot effects as `HomeEvent`s; one declarative `HomeScreen`; preferences behind repositories;
**zero `AsyncTask`/`Handler` in the Home path**; and **zero fragments** — no `FragmentManager` use in Home,
and the seven list/search fragments (plus the vendored `ListFragment.java`) deleted app-wide via P11b.
This closes the Home-sized slice of debt findings #2 and #6. Deliberately retained (documented as
decisions, each tied to a separate campaign): the imperative map *view* + its `onSaveInstanceState`
camera bundle, the one-frame arrivals-response courier, the region-found toast, `GoogleApiClient`
(PR #1569), and the SurveyManager rewrite.
...
### P17 — Idiomatic MVVM for the Home path  *(LANDED — google device-verified, maplibre compile-verified; done in 3 commits P17a/b/c)*

> **As built — 2026-06-12 (the bold version).** On the user's steer ("leave the codebase in a modern
> idiomatic Kotlin MVVM state, even if it's a fair amount of work"), P17 went beyond the mechanical
> 3-field hoist below: Home's coordination is now genuinely unidirectional. The ViewModel owns the
> coordination state and emits one-shot `HomeEvent`s the Activity applies to its imperative map view;
> the map *view* stays an `AndroidView` (a boundary, not debt). The line drawn is **trigger provenance** —
> VM-owned timing/decisions become events (sheet-settled → `SetMapPadding`/`RecenterOnFocusedStop`;
> restore/deep-link focus → `CompletePendingMapFocus`; "show vehicles on map" → `CollapseSheet`+`ShowRouteOnMap`;
> back-clear → `ClearMapFocus`), while raw user gestures on the map (zoom, my-location, layer toggle) stay
> direct imperative calls. The one real boundary — `setFocusStop` needs the arrivals response's raw
> io/elements `ObaStop`+routes the VM is decoupled from — is handled by a **one-frame courier**: the VM owns
> the pending-focus latch + the animate decision + the event, and `onArrivalsLoaded()` returns whether to
> hand the response to the collector (rather than faking a decouple that would drag in the map-overlay
> campaign). Startup: `mInitialStartup` + its `"initialStartup"` pref moved behind a `StartupPreferencesRepository`,
> with the VM owning the gate (`onHomeStarted(hasPermission)` / `onLocationPermissionResult()`) and staying
> Android-static-free. Deleted from the Activity: `mLastSettledSheet`, `mPendingMapFocus`, `mInitialStartup`,
> `checkRegionStatus()`, `onSheetSettled`, `onShowRouteOnMap`, `onClearFocus`.
>
> Landed in three independently-verified commits: **P17a** (startup gate + repo), **P17b** (settled-sheet
> state + padding/recenter events; the parity-sensitive one), **P17c** (pending-focus courier + route mode +
> clear focus). Home JVM tests 52→67 (`HomeViewModelTest` 21→35, `HomeStateTest`+1). Both flavors compile.
> **Device-verified** (google): first-launch defers→permission-result→region check (+eager on later launches);
> the full sheet cycle (tap→peek→expand recenters + padding tracks peek; back collapses; back clears focus +
> resets padding); and **the courier across a true process death** (`am kill` → new pid → restored stop
> refocused + map recentered). **Accepted deviations** (documented below + in the design): the region-found
> toast, the `setFocusStop` courier, and the map-camera `onSaveInstanceState` stay Activity-side (they need
> Context / raw io/elements types that belong to the separate map-controller campaign #7). **Phase 2 is
> complete.** The original mechanical-hoist spec is kept below for the record.
...
P17 idiomatic MVVM for the Home path        ── large (3 commits a/b/c; bold scope)  ✅ DONE
```

**Phase 2 complete (2026-06-12).** Home's data flow is now genuinely unidirectional MVVM: the
ViewModel is the single source of truth for coordination state and emits one-shot `HomeEvent`s the
Activity applies to its (deliberately imperative) map view; preferences are behind repositories;
no `AsyncTask` and no scattered mutable coordination flags remain in the Home path.
