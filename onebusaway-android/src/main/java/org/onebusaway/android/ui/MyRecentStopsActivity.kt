 ...
 * [MyStopsActivity]'s recent tab (`tab://recent_stops`), then finishes. Kept as its own manifest
 * entry so shortcuts pinned by older versions keep resolving.
 */
class MyRecentStopsActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.action == Intent.ACTION_CREATE_SHORTCUT) {
            val shortcut = UIUtils.makeShortcutInfo(
                ...
                Intent(this, MyStopsActivity::class.java).setData(MyTabs.defaultTabUri(MyTabs.RECENT_STOPS)),
