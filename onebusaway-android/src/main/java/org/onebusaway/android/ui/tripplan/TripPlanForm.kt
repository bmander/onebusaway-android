        ...
        OutlinedTextFieldDefaults.DecorationBox(
            ...
            innerTextField = {
                InputChip(
                    selected = true,
                    onClick = onClear,
                    label = { Text(endpointText) },
                    leadingIcon = if (endpoint is TripEndpoint.Geocoded && endpoint.isTransit) {
                        { BusIcon() }
                    } else {
                        null
                    },
...
@Composable
private fun endpointLabel(endpoint: TripEndpoint): String = when (endpoint) {
    is TripEndpoint.Geocoded -> endpoint.displayName
    is TripEndpoint.AddressBook -> endpoint.displayName
    is TripEndpoint.CurrentLocation -> stringResource(R.string.tripplanner_current_location)
    is TripEndpoint.MapPoint -> stringResource(R.string.trip_plan_map_location)
    is TripEndpoint.FreeText -> endpoint.query
}
