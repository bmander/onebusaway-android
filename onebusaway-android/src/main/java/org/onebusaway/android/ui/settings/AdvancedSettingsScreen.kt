    ...
    LaunchedEffect(viewModel) {
        viewModel.effects.collect { effect ->
            when (effect) {
                ...
                AdvancedSettingsEffect.OtpUrlChanged -> onOtpUrlChanged()
        ...
        onObaUrlChange = { url ->
            when (viewModel.onCustomObaApiUrlChanged(url)) {
                UrlChangeResult.InvalidObaUrl -> {
                    Toast.makeText(
                        context,
                        context.getString(R.string.custom_api_url_error, appName),
                        Toast.LENGTH_SHORT,
                    ).show()
                    false
                }
        ...
        onOtpUrlChange = { url ->
            when (viewModel.onCustomOtpApiUrlChanged(url)) {
                UrlChangeResult.InvalidOtpUrl -> {
                    Toast.makeText(
                        context,
                        context.getString(R.string.custom_otp_api_url_error),
                        Toast.LENGTH_SHORT,
                    ).show()
                    false
                }
